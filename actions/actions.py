...
from actions.account_service import AccountService
...
class ValidatePhoneNumberForm(FormValidationAction):
    ...
    def validate_phone_number(
        ...
        account_info = AccountService.check_by_phone_number(phone_number)

        AccountService.send_otp_number(phone_number)

        if account_info == 404 or account_info == 400:
            dispatcher.utter_message(text="The phone number is not valid.")
            return {"phone_number": None}

        else:
            dispatcher.utter_message(text="An OTP was sent to your phone number.")
            return {"phone_number": phone_number}
            
        return []


class ValidateOTPNumberForm(FormValidationAction):
    def name(self) -> Text:
        return "validate_otp_number_form"
    ...
    def validate_otp_number(
        ...
        #     """Validate otp number value."""

        phone_number = tracker.get_slot('phone_number')
        otp_number = tracker.get_slot('otp_number')

        otp_response = AccountService.verify_otp_number(phone_number, otp_number)

        if otp_response == 400 or otp_response == 404:

            dispatcher.utter_message(text="This OTP is not valid.")
            return {"otp_number": None}

        else:
            account_info = AccountService.check_by_phone_number(phone_number)

            dispatcher.utter_message(text="OTP is verified\nYour Account Name: {}\nYour Account Number: {}".format(account_info['name'], account_info['account_no']))
